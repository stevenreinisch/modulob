...
- (void) dispatchEvent: (MOBEvent) event {
	...
	case PasswordEntry_Event_DIGITENTERED:
	{
		...
		{
			if ([self evaluateGuardSelector: @"guard_PartiallyFilled_to_CompletelyFilled"]) {
				[self switchTransitionWithID: PasswordEntry_Transition_PARTIALLYFILLED_COMPLETELYFILLED];
			}
			else if ([self evaluateGuardSelector: @"guard_PartiallyFilled_to_PartiallyFilled_DigitEntered"]) {
	...
	case PasswordEntry_Event_USERNOTAUTHENTICATED:
	{
		...
		{
			if ([self evaluateGuardSelector: @"guard_CompletelyFilled_to_Locked"]) {
				[self switchTransitionWithID: PasswordEntry_Transition_COMPLETELYFILLED_LOCKED];
			}
			else if ([self evaluateGuardSelector: @"guard_CompletelyFilled_to_Empty"]) {
